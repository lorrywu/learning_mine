...
class Node(object):
    ...
    def calc_hidden_layer_delta(self):
        """
        节点属于隐藏层时, 根据 隐藏层节点的误差方法,计算误差 ai(1-ai) (求和 wki * detak)
        :return:
        """
        downstream_dela = reduce(lambda ret, conn: ret + conn.downstream_node.delta * conn.weight, self.downstream, 0.0)
        self.delta = self.output * (1- self.output) * downstream_dela
...
class Network(object):
    ...
    def train(self, labels, data_set, rate, iteration):
        """
        训练神经网络
        :param labels: 数组,训练样本标签, 每个元素是一个标签
        :param data_set: 二维素组,训练样本特征,每个元素是一个训练的特征
        :param rate: 学习速率
...
class Normalizer(object):
    ...
    def norm(self, number):
        map_list = map(lambda m: 0.9 if number & m else 0.1, self.mask)
        print(list(map_list))
